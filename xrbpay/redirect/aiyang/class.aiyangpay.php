...
class aiyangpay{
	const aiyang_card_url			  = 'http://api.aiyangpay.com/card.ashx';
	const aiyang_card_search_url	= 'http://api.aiyangpay.com/search.ashx';
	...
	*/
	public function search($order_id){
		...
		//签名
		$sign	= md5($url. $this->key);
		$url	= ekapay::aiyang_card_search_url . "?" . $url . "&sign=" .$sign;		
		$result=file_get_contents($url);
		parse_str($result, $output);
