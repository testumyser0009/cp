var StatGame={
		...
		$.ajax({
			...
			success:function(game_stats){
				if(game_stats){
					...
					$("#qiu1_zhws").html(game_stats["zhwsdx"]);
					$("#qiu1_lh").html(game_stats["lh"]);
				}
			}
		});
	},
	showStat:function(qiuNum){
		...
		if(statType=="zhdx"||statType=="zhds"||statType=="zhws"||statType=="lh"){
			$("#qiu1_"+statType).show();
	...
	showStatType:function(dom){
		...
		if(curType=="zhdx"||curType=="zhds"||curType=="zhws"||curType=="lh"){
			$("#qiu1_"+curType).show();
		}else{
