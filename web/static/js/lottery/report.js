var StatBet = {
	getStatDate : function(pageOption) {
		...
		$.ajax({
					type : "GET",
					url : "/daili/report.do?t=" + Date.parse(new Date()),
					dataType : "json",
					data:pageOption,
					success : function(result) {
						if (result) {
							var htmlStrFormat = '<tr><td>{statDate}</td><td>{userName}</td><td>{type}</td><td>{betCount}</td><td>{betMoney}</td><td>{zjAmount}</td><td>{reward}</td><td>{rewardRebate}</td><td><a target="framePage" href="/game/stat/statTotalBet.html?date={statDate}&settled=true">查看详情</a></td></tr>';
							var htmlStr = "";
