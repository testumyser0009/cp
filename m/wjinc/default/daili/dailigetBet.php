...
$sql=$sql." and actionTime >= ".strtotime('00:00:00')." ";
$sql=$sql.' order by id desc';
if($this->user['type']==1){
$list=$this->getPage("select * from {$this->prename}bets where  isDelete=0 and uid in (select uid from {$this->prename}members where  isDelete=0 and parentId={$this->user['uid']}) {$sql}",$pagenum,$pagesize);
}elseif($this->user['type']==2){
$list=$this->getPage("select * from {$this->prename}bets where  isDelete=0 and uid in (select uid from {$this->prename}members where  isDelete=0 and zparentId={$this->user['uid']} {$sql}",$pagenum,$pagesize);
}elseif($this->user['type']==3){
$list=$this->getPage("select * from {$this->prename}bets where  isDelete=0 and uid in (select uid from {$this->prename}members where  isDelete=0 and gudongId={$this->user['uid']} {$sql}",$pagenum,$pagesize);
}else{
exit;	
}
...
if($settled=='true'){
	//已结算明细
	$dataarr['status']=1;
	$dataarr['betmoney']=$tzmoney;
	$dataarr['rebateMoney']=$tzmoney*$var['rebate']; //已退水金额
	$allarr['otherData']['totalRebateMoney']+=$tzmoney*$var['rebate']; //退水总计
	$dataarr['resultMoney']=$var['bonus']-$tzmoney+$tzmoney*$var['rebate']; //未结明细为可赢金额//已结明细为赢亏结果包括退水

}else{
	//未结算明细
	$dataarr['status']=0;	
	$dataarr['betmoney']=$tzmoney;
	$dataarr['resultMoney']=$var['money']*$var['odds']-$tzmoney+$tzmoney*$var['rebate'];//未结明细可赢额金包括退水//已结明细结果包括退水
