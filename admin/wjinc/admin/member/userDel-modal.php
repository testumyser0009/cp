<?php 
		//检查是否有下级，并且有帐变
		$uid=$args[0];
		$son=$this->getRow("select count(*) teamNum, sum(coin) teamCoin, sum(fcoin) teamFcoin from {$this->prename}members where gudongId=$uid or zparentId=$uid or parentId=$uid");
